import logging, re, json, base64, quickjs, threading, uuid, random, proxy, logging
...
class Client:
    ...
    def request_wrapped(self, method, *args, **kwargs):
        got_response = False
        max_retries = 5
        ...
        headers = {
            ...
            **{
                "User-Agent": self.headers["User-Agent"].format(
                    rand1=random.randint(0, 9999), rand2=random.randint(0, 9999)
                ),
                "Accept-Encoding": "gzip, deflate, br",
    ...
    def generate(self, model_id: str, prompt: str, params={}):
    ...
    def chat(self, model_id: str, messages, params={}):
        ...
        logger.info("Waiting for response")
        return self.request_wrapped(
            self.session.post, self.chat_url, headers=headers, json=payload
        )
